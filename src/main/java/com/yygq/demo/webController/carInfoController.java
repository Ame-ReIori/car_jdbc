...
public class carInfoController {
    ...
    public String makeOrder(@RequestBody Order order){
        try {
            ...
            Long current_mon = userService.getUser(order.getSalerId()).getMoney()+order.getDealPrice();
            user.setMoney(current_mon);
            userService.update(user,order.getSalerId());


            //update the state of car
            Car car = carService.getCar(order.getCarId());
            car.setIsSold(true);
            car.setOwnerId(order.getCustomerId());
